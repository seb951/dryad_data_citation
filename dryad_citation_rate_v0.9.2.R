...
for(f in 1:7)
{
	if(file.exists(paste("dryad_datasets_2010_2016_update_02_2017/",full[f],"_all.html",sep = "")) == F) print(paste("Please get the dryad info for year",full[f]))
	###get info of interest from large file using Unix grep.
  #i'll be easier to grep on authors for 3 lines extra...
	system(paste("grep 'span class=\"author\"' -A 3 dryad_datasets_2010_2016_update_02_2017/",full[f], "_all.html >dryad_datasets_2010_2016_update_02_2017/info_",full[f],sep = ""))
	
	###load data in R
	info = read.table(paste("dryad_datasets_2010_2016_update_02_2017/info_",full[f],sep = ""), stringsAsFactors = F,sep = "\t", quote = "")
...
#for(j in c(1,3,5,6))
{
	...
	system(paste("ls -1 webofscience_update_02_2017/",gsub(" ","_",tolower(journals[j]))," >webofscience2017/temp_list",sep =""))
	
	#load the names of all the files 
	temp_list = read.table("webofscience_update_02_2017/temp_list",stringsAsFactors = F,header = F)
	...
	for(t in 1:nrow(temp_list))
	{
		...
		system(paste("awk -F \"\\t\" 'BEGIN { OFS = \"\\t\" } {print $2,$9,$10, $33, $45}' webofscience_update_02_2017/",gsub(" ","_",tolower(journals[j])),"/",temp_list[t,1]," > webofscience_update_02_2017/temp",t,sep = ""))
	
		#load it
		temp_journal_webofscience_sub = read.table(paste("webofscience_update_02_2017/temp",t,sep = ""),header = T, sep = "\t",stringsAsFactors = F,quote = "")
...
for(j in 1:10)
{
  ...
  points(x = full[1:7], y = dryad_rise[1:7,j],type = "b",col = coco[j], lwd = 4,pch = c(rep(1,5),rep(2,5))[j])
  if(j == 10) legend(x = 2010, y = 1, legend = journals, cex = 0.75,pch = c(rep(1,5),rep(2,5)), col = coco, lwd = 2.4)  
}
